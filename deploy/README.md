...
推送 `main` 或手动运行 GitHub Actions 中的 `Deploy production` 后，工作流会执行后端测试、构建前端、上传发布包、替换服务器产物并检查 `/api/health`。健康检查失败时，服务器脚本会自动恢复上一版 JAR 和前端文件。
...
日常发布只需将经过验证的代码推送到 `main`。同一时间只允许一个生产发布，新的提交不会中断正在执行的发布。
