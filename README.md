...
识海不是简单的文件存储工具，而是一套以知识空间为核心的个人知识管理与 AI 学习系统。用户可以将资料导入个人、团队、组织或公共空间，系统自动完成解析、OCR、内容切分、向量化和索引，再通过权限过滤后的 RAG 检索为 AI 问答提供依据。

### 知识空间

知识空间是文档、对话、学习记录和检索权限的隔离边界，当前支持四种类型：

- `PERSONAL`：每个用户自动拥有的个人空间，仅本人可读写。
- `TEAM`：团队成员共享的空间，支持所有者、编辑者和查看者角色。
- `ORG`：组织根空间，可以在组织下创建团队；组织级共享资料对下属团队可见。
- `PUBLIC`：平台公共知识源，所有用户可以在 AI 问答中检索，维护权限由管理员控制。

团队成员可以读取当前团队及其祖先组织的共享资料；组织成员可以读取组织本身及下属团队的资料。不同组织、团队和个人空间之间相互隔离，后端会在文档列表、原始文件、会话和 RAG 检索环节重复执行权限校验。

### 文档处理流程

上传文档后，系统会先将原始文件保存到当前空间，再创建后台任务，不需要用户一直等待完整的解析和索引过程。任务会依次完成文件解析、OCR（需要时）、内容切分、向量生成和索引持久化，完成后文档即可参与检索。

支持的资料类型包括 Markdown、TXT、HTML、PDF、DOC/DOCX、XLS/XLSX、PPT/PPTX、CSV、JSON/JSONL、XML、RTF、ODT，以及 PNG、JPG/JPEG 图片。单文件默认限制为 50 MB，图片和扫描 PDF 支持 OCR。

解析结果会保留文件名、来源路径、标题层级、页码、文本位置、chunk 序号、所属用户、所属空间和可见性等元数据。Markdown、PDF、Word、HTML、图片和普通文本会根据自身结构选择对应的解析器和切分器，便于检索时定位具体章节、页面或文本片段。
...
```text
原始资料 → AI 提问与学习 → 学习记录 → 正式笔记 → 再次进入检索
```

原始资料适合作为外部知识来源，正式笔记则是用户理解后的结构化成果。文档索引和向量库属于可重建的检索投影，核心学习记录、正式笔记和教学资产持久化在 PostgreSQL 中。

### 任务与运维能力

文档任务支持排队、处理中、等待重试、处理完成和处理失败等状态，并展示解析、OCR、切分、向量化和索引保存等阶段。长 PDF 支持分批处理、批次进度、成功批次跳过恢复、失败原因提示和可恢复任务重试；确定性的文件大小、页数或 OCR 限制错误需要先处理原文件。

知识库管理页面支持按文件名、路径、文件类型和文档类别筛选，查看文档内容、打开源文件、删除文档、查看上传历史、查看索引任务，以及进行目录导入、空间同步和索引重建等维护操作。

### 知识库能力概括

识海知识库具备以下核心能力：

- 多空间隔离：个人、团队、组织和公共知识空间。
- 多格式导入：办公文档、结构化数据、网页内容和图片 OCR。
- 异步索引：上传、解析、切分、向量化和任务状态解耦。
- 混合检索：语义向量检索结合 PostgreSQL 关键词检索。
- 权限检索：后端按用户、角色、空间层级和文档可见性过滤。
- 来源追踪：保留文件、章节、页码、位置和 chunk 元数据。
- 学习闭环：从原始资料到学习记录和正式笔记的持续沉淀。
- 可运维性：任务进度、失败重试、批次恢复、同步和索引重建。
- 安全控制：路径限制、文件限制、会话认证、审计和 RAG 提示词防注入。
...
访问前端地址时，未登录用户会先看到公开产品首页，可以直接了解识海的知识库能力、空间模型和资料处理流程。点击“登录”“开始使用”或“免费开始使用”后，登录和注册表单会以弹窗方式打开；认证成功后才进入个人工作台。已登录用户访问网站时会自动恢复会话并进入工作台。
