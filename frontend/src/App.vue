...
const SupportAgentPanel = defineAsyncComponent(() => import('./components/SupportAgentPanel.vue'))
  ...
  <template v-else-if="currentUser">
    <div class="learning-portal" :class="{ 'content-page': activeSection !== 'assistant' }" @keydown.esc="mobileNavOpen = false">
      ...
      <WorkspaceManager v-if="workspaceManagerOpen" :workspace="workspaces.find(item => item.id === activeWorkspaceId)" :can-create-public="currentUser.systemRole === 'ADMIN' || currentUser.systemRole === 'SUPER_ADMIN'" :can-create-org="currentUser.systemRole === 'SUPER_ADMIN'" @close="workspaceManagerOpen = false" @created="handleWorkspaceCreated" />
        ...
        <SupportAgentPanel v-if="currentUser" :key="`${currentUser.account}-${currentUser.systemRole}-${activeWorkspaceId}`" :workspace="workspaces.find(item => item.id === activeWorkspaceId)" :current-user="currentUser" />
